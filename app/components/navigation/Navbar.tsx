...
export default function Navbar({ links }: NavbarProps) {
  ...
  return (
    <motion.div
      ...
      <div className="flex m-3 justify-end items-center gap-3 sm:gap-8">
        ...
        {links.map(({ to, label, external }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className={`text-xs sm:text-sm md:text-base ${isActive ? "font-semibold" : "font-normal"}`}>{label}</span>
                {isActive && (
                  <span
